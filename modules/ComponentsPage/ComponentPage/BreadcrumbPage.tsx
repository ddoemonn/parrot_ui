    ...
    <nav aria-label="breadcrumb" className="text-md">
      <ol className="flex flex-wrap list-none p-0 m-0">
        {items.map((item, index) => (
          ...
          >
            {item.href ? (
              <a href={item.href} className={\`\${activeTab === item.href ? 'text-indigo-500' : ''}\`}>
                {item.label}
              </a>
            ) : (
...
const breadcrumbUsage = `export default function BreadcrumbPage() {
...
export default function BadgePage() {
  ...
  return (
    <ComponentDetail
      ...
      name="Breadcrumb"
      detail="Displays a breadcrumb navigation for a website."
    />
  );
}
